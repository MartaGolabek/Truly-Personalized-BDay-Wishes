...
def main():
    ...
    __website_address = "https://www.holidaycardsapp.com/wishes/best-happy-birthday-wishes-for-friends/"
    r = requests.get(__website_address)

    doc = BeautifulSoup(r.text, "html.parser")

    raw_wishes = ["text"]
    for record in doc.find_all("font"):
        if re.match("\d+\.\s", record.text) is not None:
            temp_1 = re.sub(r"\d+\.", "", record.text).strip()
            raw_wishes.append(temp_1)

    with open("../../data/raw/bday_wishes_for_friend_3.csv", "w", newline="") as file:
