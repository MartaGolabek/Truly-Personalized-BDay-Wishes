...
def main():
    ...
    __website_address = "https://www.happybirthdaymsg.com/funny-birthday-wishes-for-best-friend/"
    ...
    for record in doc.select("li"):
        if re.match("\d\s", record.text) is None:
            if record.text and any(c.isalpha() for c in record.text):
                raw_wishes.append(record.text)

    print(len(raw_wishes))

    with open("../../data/raw/bday_wishes_for_friend_7.csv", "w", newline="") as file:
        csv_writer = csv.writer(file, delimiter=";")
        raw_wishes_extracted = raw_wishes[6:99]
